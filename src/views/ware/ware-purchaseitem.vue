...
import { purchaseDetailList, deleteIds, mearge } from '@/api/ware/purchase-detail'
import { undeceiveList } from '@/api/ware/purchase'
...
export default {
  ...
  methods: {
    mergeItem() {
      ...
      if (!this.purchaseId) {
        this.$confirm(
          ...
          .then(() => {
            mearge({ items: items }).then((response) => {
              this.getDataList()
            })
          })
      } else {
        mearge({ purchaseId: this.purchaseId, items: items }).then((response) => {
          this.getDataList()
        })
      }
    ...
    getDataList() {
      ...
      purchaseDetailList(this.pageIndex, this.pageSize, this.dataForm.key, this.dataForm.status, this.dataForm.wareId).then((response) => {
